...
with open("results.txt", "w") as f:
    for element in tqdm(ds):
        for limit in limit_range:
            c = conn.execute(f"SELECT * FROM openai3072 ORDER BY (text_embedding_3_large_3072_embedding[0:256])::vector(256) <-> '%s' LIMIT {limit}" % ("[" + ", ".join([str(e) for e in element[embedding_column_name][0:256]]) + "]"))
            records = c.fetchall()
            hnsw256 = [item[0] for item in records]
        
            c = conn.execute(f"SELECT * FROM openai3072 ORDER BY (text_embedding_3_large_3072_embedding[0:1024])::vector(1024) <-> '%s' LIMIT {limit}" % ("[" + ", ".join([str(e) for e in element[embedding_column_name][0:1024]]) + "]"))
            records = c.fetchall()
